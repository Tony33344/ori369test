...
async function createShop() {
  ...
  try {
    ...
    // First, just try to insert into the tables to see if they exist
    const { error: testErr } = await supabase
      .from('shop_categories')
      .select('id')
      .limit(1);

    if (testErr && testErr.message.includes('relation')) {
      console.log('⚠️  Tables do not exist. You need to:');
      console.log('1. Go to: https://app.supabase.com/project/YOUR_PROJECT/sql/new');
      console.log('2. Paste this SQL and run it:\n');
    ...
    for (const cat of categories) {
      ...
      if (products[cat.slug] && catData?.id) {
    ...
    console.log(`\n✨ Done! Seeded ${totalProducts} products`);
    process.exit(0);
  } catch (error) {
